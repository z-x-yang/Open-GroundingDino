...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, cat_list, caption, return_indices=False):
        ...
        one_hot = torch.zeros(outputs['pred_logits'].size(
        ), dtype=torch.int64)  # torch.Size([bs, 900, 256])
        token = outputs['token']
        ...
        for j in range(len(cat_list)):  # bs
            ...
            for i in range(len(cat_list[j])):
                ...
                per_label = create_positive_map(
                    token[j], label_id, cat_list[j], caption[j])
        ...
        if 'aux_outputs' in outputs:
            for idx, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for j in range(len(cat_list)):  # bs
                    ...
                    inds = self.matcher(aux_output_single, [
                                        targets[j]], label_map_list[j])
                    indices.extend(inds)
                ...
                for i in range(len(indices)):
                    tgt_ids[i] = tgt_ids[i][indices[i][1]]
                    one_hot_aux[i, indices[i][0]
                                ] = label_map_list[i][tgt_ids[i]].to(torch.long)
        ...
        if 'interm_outputs' in outputs:
            ...
            for j in range(len(cat_list)):  # bs
                ...
                inds = self.matcher(interm_output_single, [
                                    targets[j]], label_map_list[j])
                indices.extend(inds)
            one_hot_aux = torch.zeros(
                outputs['pred_logits'].size(), dtype=torch.int64)
...
class PostProcess(nn.Module):
    ...
    def __init__(self, num_select=100, text_encoder_type='text_encoder_type', nms_iou_threshold=-1, use_coco_eval=False, args=None) -> None:
        ...
        # Note: Do not remap to COCO-91 id space. Use dataset categories directly.
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False):
        ...
        for label_ind in range(len(pos_maps)):
            if pos_maps[label_ind].sum() != 0:
                pos_maps[label_ind] = pos_maps[label_ind] / \
                    pos_maps[label_ind].sum()
...
def build_groundingdino(args):
    ...
    postprocessors = {'bbox': PostProcess(
        num_select=args.num_select, text_encoder_type=args.text_encoder_type, nms_iou_threshold=args.nms_iou_threshold, args=args)}
